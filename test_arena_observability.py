...
class ArenaObservabilityTests(unittest.TestCase):
    ...
    def test_observation_removes_identifiers_and_unapproved_event_values(self) -> None:
        ...
        self.assertEqual(observation["strategy"]["state"], "CONSOLIDATE")
        self.assertEqual(observation["strategy"]["population_health"], "CONSOLIDATING")
        self.assertEqual(observation["strategy"]["committed_population"], 0)
        self.assertEqual(observation["strategy"]["production_ceiling"], 24)
        self.assertEqual(observation["strategy"]["population_limit"], 24)
        self.assertIn("migration_shadow", observation["strategy"])
        self.assertEqual(observation["strategy"]["migration_shadow"]["status"], "NOT_EVALUATED")
